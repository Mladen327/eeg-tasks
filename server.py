"""Server za demonstrator Scenarija 1.

Radi dve stvari istovremeno (identicno s2-demo/server.py i
s3-demo/server.py po strukturi -- za razliku od njih, ovaj sluzi ceo
eeg-tasks/ (core/, shared/, data/, app/, scenarios/), jer S1 nema
sopstveni projekat, SPEC_refaktor_jezgro.md):

1. Sluzi staticke fajlove (app/, core/, scenarios/, data/) kao obican
   HTTP server.
2. Prima WebSocket vezu od scenarios/s1.js (ws://<host>:8767, RAZLICIT
   port od s2-demo (8766) i s3-demo (8765), sva tri mogu da rade
   istovremeno) i za svaku primljenu poruku:
     - upisuje red u odgovarajucu .jsonl datoteku u logs/ (ili
       logs/practice/ za variant=="practice"), SPEC_S1_demo.md sekcija 8
     - ako je pylsl dostupan, prosledjuje dogadjaj kao LSL marker u tok
       "S1_markers" (tip "Markers")

Ako pylsl nije instaliran, server nastavlja da radi normalno.

Pokretanje:
    python server.py                    # http :8000, ws :8767
    python server.py --http-port 8080 --ws-port 8767
"""
...
class LslForwarder:
    def __init__(self):
        ...
        self._warned = False
        if HAVE_LSL:
            info = StreamInfo("S1_markers", "Markers", 1, 0, "string", "s1-demo-markers")
            self.outlet = StreamOutlet(info)
            print("[lsl] LSL tok 'S1_markers' pokrenut")
        else:
...
class ApiHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _session_count(self, parsed):
        ...
        count = 0
        # DEMO nema brojanje sesija (core/intro.js je nikad i ne pita).
...
async def run_ws_server(port: int):
    if websockets is None:
        print("[ws] UPOZORENJE: paket 'websockets' nije instaliran (pip install websockets). "
              "Log preko WebSocket-a nece raditi; scenarios/s1.js ce pasti na preuzimanje "
